...
def load_csv_files():
    ...
    try:
        ...
        area_category = pd.read_csv(
            os.path.join(data_dir, "area_category.csv")
        )
...
def analyze_data(area_map, area_struct, area_category):
    ...
    print("\n첫 5행:")
    print(area_map.head())
    print(f"\n건설현장 개수: {area_map['Construct'].sum()}개")
...
def convert_category_to_name(area_struct, area_category):
    ...
    # 카테고리 매핑 딕셔너리 생성
    category_map = dict(
        zip(area_category["category"], area_category[" struct"])
    )
...
def merge_dataframes(area_map, area_struct):
    ...
    # area 기준으로 정렬
    merged_df = merged_df.sort_values(["area", "x", "y"]).reset_index(
        drop=True
    )
...
def find_special_structures(merged_df):
    ...
    # MyHome 찾기
    myhome_data = merged_df[
        merged_df["struct"].str.contains("MyHome", na=False)
    ]
    if not myhome_data.empty:
        for _, row in myhome_data.iterrows():
            print(
                f"MyHome 위치: ({row['x']}, {row['y']}) - area {row['area']}"
            )
    else:
        print("MyHome을 찾을 수 없습니다.")

    # 반달곰 커피 찾기
    coffee_data = merged_df[
        merged_df["struct"].str.contains("BandalgomCoffee", na=False)
    ]
    if not coffee_data.empty:
        print(f"\n반달곰 커피 총 {len(coffee_data)}개 발견:")
        for _, row in coffee_data.iterrows():
            print(
                f"  - 위치: ({row['x']}, {row['y']}) - area {row['area']}"
            )

    # 아파트 찾기
    apartment_data = merged_df[
        merged_df["struct"].str.contains("Apartment", na=False)
    ]
    ...
    # 빌딩 찾기
    building_data = merged_df[
        merged_df["struct"].str.contains("Building", na=False)
    ]
...
def analyze_by_area(merged_df, target_area=None):
    ...
    if target_area is None:
        ...
        for area in sorted(merged_df["area"].unique()):
            ...
            # 건설현장 개수
            construct_count = area_df["Construct"].sum()
            print(f"건설현장: {construct_count}개")
    else:
        # 특정 지역만 분석
        ...
        # 건설현장 통계
        construct_stats = area_df["Construct"].value_counts()
        print("\n건설현장 통계:")
        print(f"  - 건설현장: {construct_stats.get(1, 0)}개")
        print(f"  - 일반 지역: {construct_stats.get(0, 0)}개")
...
def generate_comprehensive_report(merged_df):
    ...
    total_construct = merged_df["Construct"].sum()

    print("\n전체 통계:")
    print(f"  - 총 격자 수: {total_grids}개")
    print(f"  - 총 지역 수: {total_areas}개")
    print(f"  - 총 건설현장: {total_construct}개")
    ...
    for area in sorted(merged_df["area"].unique()):
        ...
        construct_count = area_df["Construct"].sum()
        print(
            f"  - Area {area}: 격자 {len(area_df)}개, "
            f"특별구조물 {special_count}개, 건설현장 {construct_count}개"
        )
...
def main():
    ...
    try:
        ...
        print("추가 분석 옵션:")
        print(
            "특정 지역만 분석하려면 analyze_by_area(merged_df, target_area=숫자) "
            "함수를 사용하세요."
        )
